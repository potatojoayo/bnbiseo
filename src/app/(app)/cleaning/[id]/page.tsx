...
const STATUS_CONFIG: Record<string, { label: string; color: string; step: number }> = {
  pending: { label: '매니저 배정 중', color: 'bg-[#FFF1F3] text-brand', step: 1 },
  confirmed: { label: '매니저 배정 완료', color: 'bg-[#E8F5E9] text-[#2E7D32]', step: 2 },
  in_progress: { label: '청소 진행 중', color: 'bg-[#E3F2FD] text-[#1565C0]', step: 3 },
  completed: { label: '청소 완료', color: 'bg-[#F7F7F7] text-[#222222]', step: 4 },
  cancelled: { label: '취소됨', color: 'bg-[#F7F7F7] text-[#B0B0B0]', step: 0 },
}
  ...
  if (!cleaning) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100dvh-80px)] px-6 text-center">
        ...
        <Link
          href="/home"
          className="mt-4 text-[14px] text-[#717171] underline underline-offset-2"
        >
          홈으로
        </Link>
      </div>
  ...
  return (
    <div className="animate-fade-up-fast min-h-[calc(100dvh-80px)] flex flex-col px-6 pt-6 pb-10">
      ...
      {cleaning.propertyName && (
        <div className="rounded-xl border border-[#EBEBEB] px-4 py-4 flex flex-col gap-2.5 mb-4">
      ...
      {/* Cleaning details */}
      <div className="rounded-xl border border-[#EBEBEB] px-4 py-4 flex flex-col gap-2.5 mb-6">
        <div className="flex items-center gap-2.5">
        ...
        {cleaning.memo && (
          <div className="border-t border-[#EBEBEB] pt-2.5 mt-0.5">
            <p className="text-[13px] text-[#717171]">{cleaning.memo}</p>
          </div>
        )}
      </div>
      ...
      {cleaning.status !== 'cancelled' && (
        <>
          <p className="text-[13px] font-medium text-[#717171] mb-3 px-1">진행 과정</p>
                ...
                <div key={step.num} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    <div className={cn(
                      ...
                      isDone ? 'bg-brand text-white' : 'bg-[#EBEBEB] text-[#B0B0B0]'
                    )}>
                      {step.num}
                    </div>
                    {i < PROCESS_STEPS.length - 1 && (
                      <div className={cn(
                        'w-px flex-1 my-1',
                        isDone && i < currentStep - 1 ? 'bg-brand' : 'bg-[#EBEBEB]'
                      )} />
                    )}
                  </div>
                  <div className="pb-5">
                    <p className={cn(
                      ...
                      isDone ? 'text-[#222222]' : 'text-[#B0B0B0]'
                    )}>
                      {step.title}
                      {isCurrent && isDone && (
                    ...
                    <p className={cn(
                      ...
                      isDone ? 'text-[#717171]' : 'text-[#D0D0D0]'
                    )}>
                      {step.desc}
                    </p>
                  </div>
      ...
      {canCancel && (
        <button
          ...
          className="text-[13px] text-[#B0B0B0] underline underline-offset-2 hover:text-[#717171] transition-colors self-center mt-auto"
        >
          청소 요청 취소
        </button>
      )}
