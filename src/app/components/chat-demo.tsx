          ...
          >
            <div
              className={`px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-line max-w-[85%] font-text ${
                m.f === "u"
                  ? "bg-brand text-white rounded-t-xl rounded-bl-xl rounded-br-sm"
                  : "bg-surface-dim text-on-surface rounded-t-xl rounded-br-xl rounded-bl-sm"
              }`}
            >
              {m.t}
            </div>
            {m.opts && (
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {m.opts.map((o, j) => (
                  <span
                    key={j}
                    className="px-3 py-1 rounded-full border border-outline-dim text-on-surface-subtle text-xs font-text"
                  >
                    {o}
                  </span>
                ))}
          ...
          >
            <div
              className={`rounded-xl px-4 py-2.5 flex gap-1 text-xs ${
                msgs[step].f === "u"
                  ? "bg-brand/80 text-white/60"
                  : "bg-surface-dim text-on-surface-subtle"
              }`}
            >
