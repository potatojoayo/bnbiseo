...
export type UploadedRepairImage = {
  storagePath: string
...
async function uploadToSignedPaths(
  file: File,
  signed: { original: { path: string; token: string }; thumbnail: { path: string; token: string } },
): Promise<UploadedRepairImage> {
...
// 수리 요청 생성 전 호스트 사진 업로드 (orphan upload)
export async function uploadRepairRequestImage(file: File) {
  const signed = await api.post<{
    original: { path: string; token: string }
    thumbnail: { path: string; token: string }
  }>('/repair/photos/upload-url', { fileName: file.name })

  return uploadToSignedPaths(file, signed)
}

// 매니저 완료 보고서 사진 업로드
export async function uploadRepairCompletionImage(repairId: string, file: File) {
  const signed = await api.post<{
    original: { path: string; token: string }
    thumbnail: { path: string; token: string }
  }>(`/manager/repairs/${repairId}/complete-report/upload-url`, { fileName: file.name })
