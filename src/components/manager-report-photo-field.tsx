...
import {
  type UploadedManagerReportImage,
  uploadManagerReportImage,
} from '@/lib/manager-report-image-upload'
import { ApiError } from '@/lib/api-client'
import { Skeleton } from '@/components/ui/skeleton'

type ManagerReportPhotoFieldProps = {
  cleaningId: string
  assetId: string
  images: UploadedManagerReportImage[]
  onChange: (images: UploadedManagerReportImage[]) => void
  onError?: (message: string | null) => void
  readOnly?: boolean
}

export function ManagerReportPhotoField({
  cleaningId,
  assetId,
  images,
  onChange,
  ...
  async function handleFilesSelected(files: FileList | null) {
    ...
    try {
      const uploadedImages: UploadedManagerReportImage[] = []

      for (const file of Array.from(files)) {
        uploadedImages.push(await uploadManagerReportImage(cleaningId, assetId, file))
      }
  ...
  return (
    <section className="mt-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <p className="text-[13px] font-medium text-ink-muted">점검 사진</p>
        {!readOnly && (
          <button
            type="button"
